import { z } from 'zod';
import { d as derived, g as get, w as writable, a as readonly } from './index2.js';
import { ad as current_component, a0 as getContext } from './index3.js';
...
function traversePaths(parent, modifier, path = []) {
	for (const key in parent) {
		...
		if (status === 'abort') return status;
			...
			if (status2 === 'abort') return status2;
		}
	}
}
...
function unionInfo(schema) {
	if (!schema.anyOf || !schema.anyOf.length) return void 0;
...
function _defaultValues(schema, isOptional, path) {
				...
				output =
					info.union.length > 1
						? merge$1.withOptions(
								...
								...info.union.map((s) => _defaultValues(s, isOptional, path))
							)
	...
	if (info.properties) {
		for (const [key, objSchema] of Object.entries(info.properties)) {
			...
			const def =
				objectDefaults && objectDefaults[key] !== void 0
					? objectDefaults[key]
...
function mergeDefaults(parsedData, defaults) {
	if (!parsedData) return clone$1(defaults);
...
function Form(formElement, timers, options) {
	...
	const Form2 = formElement;
	{
		...
		const completed = (opts) => {
			...
			if (!opts.cancelled) setTimeout(() => scrollToFirstError(Form2, options), 1);
		};
		onDestroy(() => {
...
function superForm(form, formOptions) {
		...
		if (options.dataType !== 'json') {
			const checkForNestedData = (key, value) => {
				...
				if (Array.isArray(value)) {
					...
					throw new SuperFormError(
						`Object found in form field "${key}". Set the dataType option to "json" and add use:enhance to use nested data structures. More information: https://superforms.rocks/concepts/nested-data`
					);
				}
			};
		...
		traversePaths(errors, (error) => {
			...
			const isEventError =
				...
				paths.some((path) => {
					return isObjectError
						? path && currentPath.length > 0 && currentPath[0] == path[0]
			...
			if (isObjectError) {
				if (
					...
					(type == 'blur' && Tainted_hasBeenTainted(mergePath(error.path.slice(0, -1))))
				) {
					return addError();
				}
			} else {
	...
	function rebind(opts) {
		...
		else Errors.set(form2.errors);
		FormId.set(form2.id);
		Posted.set(opts.posted ?? form2.posted);
		if (form2.constraints) Constraints.set(form2.constraints);
		if (form2.shape) Shape.set(form2.shape);
	...
	function removeFiles(formData) {
		...
		traversePaths(formData, (data2) => {
			if (data2.value instanceof File) {
				...
				Array.isArray(data2.value) &&
				data2.value.length &&
				data2.value.every((d) => d instanceof File)
			) {
				paths.push(data2.path);
				return 'skip';
			}
...
function _constraints(info, path) {
	...
	if (info.properties) {
		const obj = {};
		for (const [key, prop] of Object.entries(info.properties)) {
			const propInfo = schemaInfo(prop, !info.required?.includes(key) || prop.default !== void 0, [
				key
			]);
...
function parseFormDataEntry(key, value, type, info) {
	if (!value) {
		...
		if (defaultValue2 !== void 0) return defaultValue2;
		if (info.isNullable) return null;
		if (info.isOptional) return void 0;
	}
	function typeError() {
