...
<a
  in:fly|global={{ y: 100, duration: 300 * i, easing: backOut }}
    ...
    "
>
<div class={`w-full h-48 overflow-hidden rounded-t bg-gray-200 dark:bg-gray-800 ${isAnimating ? "animate-pulse" : ""}`}>
	  <img class="w-full object-cover h-full opacity-0 rounded group-hover:brightness-75 dark:group-hover:brightness-50 transition duration-300" width="400" use:lazyLoad={$theme === "dark" ? project_data.img.dark : project_data.img.light} alt={project_data.name} />
  </div>
	<div class="p-5">
